...
function makeBusiness(overrides: Partial<Business> = {}): Business {
  return {
    ...
    maps_url: "https://www.google.com/maps/place/?q=place_id:ChIJ_001",
    ...overrides,
  };
}
...
// ── Helper — sends via jsonTransport and parses the captured message ──────────
...
describe("MailService", () => {
  ...
  describe("recipients", () => {
    it("puts the first email_to address in to", async () => {
      ...
      // jsonTransport serialises addresses as [{address, name}]
    ...
    it("single recipient — bcc is empty or absent", async () => {
      const file = tmpExcelFile();
      const msg = await sendAndCapture(
  ...
  describe("subject", () => {
    ...
    it("subject uses i18n fallback when key is missing in active dict", async () => {
      const file = tmpExcelFile();
      const msg = await sendAndCapture(mockConfig, [makeBusiness()], file, {});
      // Should fall back to en.json value — not empty
  ...
  describe("HTML body", () => {
    ...
    it("html contains the cities from config", async () => {
      const file = tmpExcelFile();
      const msg = await sendAndCapture(mockConfig, [makeBusiness()], file);
      expect(String(msg.html)).toContain("Milan");
      expect(String(msg.html)).toContain("Rome");
    });
  ...
  describe("attachment", () => {
    ...
    it("attachment path points to the correct file", async () => {
      ...
      // jsonTransport resolves path to content — filename is preserved
      expect(attachments[0].filename).toBe(path.basename(file));
    });
  });
  ...
  describe("error handling", () => {
    it("throws a descriptive error when the Excel file does not exist", async () => {
      ...
      await expect(
        service.send(
          "/nonexistent/path/SpotCast.xlsx",
          [makeBusiness()],
          en,
          en
        )
