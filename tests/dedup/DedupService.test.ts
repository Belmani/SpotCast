...
function makeBusiness(place_id: string, overrides: Partial<Business> = {}): Business {
  return {
    ...
    maps_url: `https://maps.here.com/?ref=${encodeURIComponent(place_id)}`,
    ...overrides,
  };
}
...
describe('DedupService', () => {
  ...
  describe('markSeen', () => {
    ...
    it('updates last_seen on second call while preserving first_seen', () => {
      ...
      expect(business.first_seen).toBe(firstSeenAfterFirst);
      expect(business.last_seen).not.toBe(lastSeenAfterFirst);
    });
